...
const BG_VISIBLE_OPACITY = 0.85;
const activeBgOpacity = (index: number) => (index === 0 ? 1 : BG_VISIBLE_OPACITY);
...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      if (bgRefs.current[0]) {
        gsap.set(bgRefs.current[0], { opacity: activeBgOpacity(0), scale: 1 });
      }
    ...
    const cssVars: CSSProperties = {
      ...
      ["--fx-text" as string]: colors.text ?? "rgba(255,255,255,0.9)",
      ["--fx-overlay" as string]: colors.overlay ?? "rgba(0,0,0,0.45)",
    ...
    return (
      ...
      >
        ...
        <style jsx>{`
          ...
          .fx-featured.active .fx-featured-title {
            opacity: 0.96;
          }
          .fx-word-mask {
